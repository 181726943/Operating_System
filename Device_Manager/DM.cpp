...
void init(){
    /* 初始化SDT和DCT */
    SDT sdt_init[4];
    struct COCT coct_init[4];
    CHCT chct_init[2];

    sdt_init[0].Dname = "display1";
    sdt_init[0].type = 'O';
    sdt_init[0].DCT.Dname = sdt_init[0].Dname;
    sdt_init[0].DCT.type = sdt_init[0].type;
    sdt_init[0].DCT.state = 0;
    sdt_init[0].DCT.coid = "co1";
    sdt.push_back(sdt_init[0]);
    sdt_init[1].Dname = "keyboard1";
    sdt_init[1].type = 'I';
    sdt_init[1].DCT.Dname = sdt_init[1].Dname;
    sdt_init[1].DCT.type = sdt_init[1].type;
    sdt_init[1].DCT.state = 0;
    sdt_init[1].DCT.coid = "co2";
    sdt.push_back(sdt_init[1]);
    sdt_init[2].Dname = "mouse1";
    sdt_init[2].type = 'I';
    sdt_init[2].DCT.Dname = sdt_init[2].Dname;
    sdt_init[2].DCT.type = sdt_init[2].type;
    sdt_init[2].DCT.state = 0;
    sdt_init[2].DCT.coid = "co3";
    sdt.push_back(sdt_init[2]);
    sdt_init[3].Dname = "printer1";
    sdt_init[3].type = 'O';
    sdt_init[3].DCT.Dname = sdt_init[3].Dname;
    sdt_init[3].DCT.type = sdt_init[3].type;
    sdt_init[3].DCT.state = 0;
    sdt_init[3].DCT.coid = "co4";
    sdt.push_back(sdt_init[3]);
    

    /* 初始化控制器控制表 */
    // coct_init[0]("col",0,"ch1");
    // strcpy((char*)&coct_init[0].controllerid,"col");
    coct_init[0].controllerid = string("co1");
    coct_init[0].state = 0;
    coct_init[0].Chid = string("ch1");
    coct.push_back(coct_init[0]);
    coct_init[1].controllerid = string("co2");
    coct_init[1].state = 0;
    coct_init[1].Chid = string("ch1");
    coct.push_back(coct_init[1]);
    coct_init[2].controllerid = string("co3");
    coct_init[2].state = 0;
    coct_init[2].Chid = string("ch2");
    coct.push_back(coct_init[2]);
    coct_init[3].controllerid = string("co4");
    coct_init[3].state = 0;
    coct_init[3].Chid = string("ch2");
    coct.push_back(coct_init[3]);

    /* 初始化通道控制表 */
    chct_init[0].channelid = "ch1";
    chct_init[0].state = 0;
    chct_init[0].COid.push_back("co1");
    chct_init[0].COid.push_back("co2");
    chct.push_back(chct_init[0]);
    chct_init[1].channelid = "ch2";
    chct_init[1].state = 0;
    chct_init[1].COid.push_back("co3");
    chct_init[1].COid.push_back("co4");
    chct.push_back(chct_init[1]);
}
...
void Delete(){
    ...
    for(int i=0; i<sdt.size(); i++){    /* 定位将要删除的设备 */
        if(sdt[i].Dname.compare(Device) == 0){  /* 找到要删除的设备 */
            dflag:if(sdt[i].DCT.state == 0){  /* 检查状态，设备未被占用 */
                ...
                for(int k=0; k<run_p.size(); k++)  /* 首先找到使用该设备的进程 */
                    if(run_p[k].device_name.compare(Device) == 0){
                ...
                for(int j=0; j<coct.size(); j++)    /* 定位设备所使用控制器的位置 */
                    if(coct[j].controllerid.compare(controller) == 0)   /* 找到该控制器，置0 */
                        coct[j].state = 0;
                if(channel.compare("ch1") == 0) /* 这两个判断语句寻找设备使用通道，找到置0 */
                    chct[0].state = 0;
                else
                    chct[1].state = 0;
                goto dflag;
